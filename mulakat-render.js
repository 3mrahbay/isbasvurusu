...
import { tarihSaatFormatla, tarihFormatla, pozisyonKategorisiBul } from './yardimci.js';
...
// TAB 4: TÜM CEVAPLAR
// ───────────────────────────────────────────────
export function cevaplarTabHTML(cevaplar) {
  ...
  Object.keys(gruplar).forEach(grupAd => {
    ...
    grup.items.sort((a, b) => a.id.localeCompare(b.id));
    
    grup.items.forEach(({ id, cevap }) => {
      let cevapHTML = '';
      
      if (typeof cevap === 'object' && cevap !== null) {
        cevapHTML = `<span class="cevap-deger">Seçim: ${cevap.secim}</span>`;
        if (cevap.neden) {
          cevapHTML += `<div class="cevap-uzun"><strong>💭 Neden:</strong> ${cevap.neden}</div>`;
        }
      } else if (typeof cevap === 'string' && cevap.length > 60) {
        // Hikaye - uzun
        cevapHTML = `<div class="cevap-uzun">${cevap}</div>`;
      } else {
        cevapHTML = `<span class="cevap-deger">${cevap}</span>`;
      }
      
      html += `
        <div class="cevap-item">
          <div class="cevap-id">${id}</div>
